...
```html
<!-- fx-lab effect: truchet — Truchet Tiles (shader) · license MIT -->
<canvas id="fx-truchet" style="display:block;width:100%;height:100%;min-height:240px"></canvas>
<script type="module">
...
const GLSL = "void main(){ vec2 res=iResolution.xy; vec2 uv=gl_FragCoord.xy/res.y*6.0; uv.x+=iTime*0.12; vec2 id=floor(uv); vec2 f=fract(uv)-0.5;\n      if(hash(id)>0.5) f.x=-f.x; float d=abs(length(f-0.5)-0.5); d=min(d,abs(length(f+0.5)-0.5)); float line=smoothstep(0.09,0.04,d);\n      vec3 col=mix(vec3(0.04,0.06,0.09),vec3(0.3,0.85,0.9),line); o=vec4(col,1.0); }";
const cv = document.getElementById('fx-truchet');
const gl = cv.getContext('webgl2');
if (!gl) { console.error('fx-truchet: WebGL2 not supported'); } else {
